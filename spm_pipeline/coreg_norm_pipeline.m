function coreg_norm_pipeline(run_everything, EPI_struct_array, EPI_field_struct_array, FLAIR_3D_struct_array, EPI_folder_name)
    ...
    % Update: parfor was not able to run within this function, hence
    % the parallelized version of this code all runs in coreg_norm_images.m

    % 1. Get files.
    % 2. Low res -> High res coreg est reslice : out: upsampled first dynamic DSC
    % 3a. Normalize est : out: deformation field
    % 3b. Normalize write : out: DSC first dynamic + topup/epic field + perfusion maps in MNI space
    % 4. Coreg reslice regions file to perfusion MNI space using
    % estimated deformation field from 3a.
    
    EPI_corrected = false;
    if strcmp(EPI_folder_name, 'EPI_applytopup') || ...
            strcmp(EPI_folder_name, 'EPI_applyepic')
        EPI_corrected = true;
    end
    ...
    for i = 1:length(EPI_struct_array)
        ...
        src_file_copied = strcat(DSC_struct_array.folder, '/_coregest_', ...
            DSC_struct_array.name);
        ...
        perfusion_dir_string = strcat(DSC_struct_array.folder, '/', ...
        ...
        %to_reslice = {};
        if EPI_corrected
            to_reslice = cell(1, length(perfusion_struct_array)+1);
        else
            to_reslice = cell(1, length(perfusion_struct_array));
        end
        
        for j = 1:length(perfusion_struct_array)
            perfusion_maps_struct_array = perfusion_struct_array(j);
            
            perfusion_file = strcat(perfusion_maps_struct_array.folder, '/', ...
                perfusion_maps_struct_array.name);
            
            perfusion_file_copied = strcat(perfusion_maps_struct_array.folder, '/_coregest_', ...
                perfusion_maps_struct_array.name);
            ...
            to_reslice{j} = strcat(perfusion_file_copied, ',1');
        end
        
        if EPI_corrected
            % The off-resonance field (topup) or deformation field (epic)
            field_copied = strcat(field_struct_array.folder, '/_coregest_', ...
                field_struct_array.name);
        ...
        to_resample{1} = strcat(DSC_struct_array.folder, '/r_coregest_', ...
            DSC_struct_array.name, ',1');
        
        % Fill in the coregistered perfusion maps
        for j = 1:length(perfusion_struct_array)
            perfusion_maps_struct_array = perfusion_struct_array(j);
            pefusion_file_copied_coregistered = strcat(perfusion_maps_struct_array.folder, '/r_coregest_', ...
                perfusion_maps_struct_array.name);
            
            %to_resample{end+1} = strcat(pefusion_file_copied_coregistered, ',1');
            to_resample{1+j} = strcat(pefusion_file_copied_coregistered, ',1');
        end
        ...
        if EPI_corrected
            ...
            to_resample{1+length(perfusion_struct_array)+1} = strcat(field_struct_array.folder, '/r_coregest_', ...
        ...
        if run_everything
            ...
            %type = determine_e1_or_e2_DSC(DSC_struct_array.name); %moved
            %above
        ...
        % A copy of this file will be resliced and saved to a separate file.
        ...
        if run_everything
            ...
            % Make a copy of the (copied) regions file, since the 
            %regions_file_copied_copied = strcat(DSC_struct_array.folder, '/_coregest_', type, '_labels_Neuromorphometrics.nii');
            %copyfile(regions_file_copied, regions_file_copied_copied);
            
            % Add regions_file_copied to the to_reslice list.
            %to_reslice{end+1} = strcat(regions_file_copied, ',1');
