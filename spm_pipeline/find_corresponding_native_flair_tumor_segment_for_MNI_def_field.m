function [matched_folder, matched_name, matched_folder_and_name] = find_corresponding_native_flair_tumor_segment_for_MNI_def_field(od, deformation_struct_array, segments_struct_array)
    ...
    derformation_dir = deformation_struct_array.folder;
    
    %disp(derformation_dir);
    %disp(segments_struct_array(1).folder);
    %disp(ONCOHabitats_tumor_segments_from_flair_native_full_path);
    ...
    for i = 1:length(segments_struct_array)
        ...
        if contains(derformation_dir, patient_id)
            %disp('BINGO!');
